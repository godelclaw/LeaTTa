...
/- SUNSET (dialect retirement): the native-PeTTa dialect surface that lived
here — prelude deltas, the petta grounding table, and the profile-specialized
prelude — is retired. PLeaTTa (PLeaTTa/, PETTA-LP.md) is the sole PeTTa
semantics: PeTTa is compilation to definite clauses + SLD resolution, not an
HE dialect. The old scoreboards (diffbench/run-t51.log) remain as regression
oracles. LeaTTa-HE below is pure HE. -/
...
def oracleReport (src : String) (fuel : Nat := 100000)
  ...
  | Except.ok atoms =>
      let res := (evalSequential atoms fuel imports importDeps profile).foldl
          (fun (acc : Nat × Nat × List String) qr =>
