...
structure MinEnv where
  ...
  /-- Evaluation dialect. LeaTTa defaults to `heProfile`; PLeaTTa selects
      `pettaProfile` through its separate executable. -/
...
def queryOp (env : MinEnv) (st : St) (prev : Stack) (toEval : Atom) (b : Bindings) : List Item × St :=
  ...
  let (results, st', _) := (candidatesW env st.world toEval).foldl
    (fun (acc : List Item × St × Bool) p =>
      ...
      let committed := env.profile.cutCommits && !mtchs.isEmpty && atomHasCut rhs'
  ...
  | top :: prev =>
    if top.fin then
      ...
      | Atom.expr (Atom.sym "get-type-space" :: args) =>
          ...
          | Atom.expr (Atom.sym op :: args) =>
              ...
              let elemTs := (Atom.sym op :: args).map (fun a =>
                ((getTypes typeEnv (typePrep st.world a)).head?).getD (Atom.sym "%Undefined%"))
              if env.profile != heProfile && !args.isEmpty
                 && !(getTypes typeEnv (typePrep st.world (Atom.sym op))).any isArrow
                 && elemTs.all (· != Atom.sym "%Undefined%") then
                ([finItem prev (Atom.expr elemTs) it.bnd], st)
              else if env.profile.typecheckStrict &&
                  (typeMismatch typeEnv st.world op args).isSome then
                ([], st)
              else emit st
          | Atom.expr (f :: args) =>
      ...
      | Atom.expr [Atom.sym "change-state!", s, v] =>
          match stateId st.world (instantiate it.bnd s) with
          | some id =>
              let result := if env.profile == heProfile then stateHandle id else env.successAtom
              ([finItem prev result it.bnd], st.mapWorld (·.setStore id (instantiate it.bnd v)))
      ...
      | Atom.expr [Atom.sym "import!", space, file] =>
          ...
          let moduleName? := match instantiate it.bnd file with
            | Atom.sym f => some f
            | Atom.expr [Atom.sym "library", Atom.sym lib] => some ("library " ++ lib)
            | _ => none
          ...
          | some "&self", some f =>
              (res, st.mapWorld (fun w => importSelfFuel env 64 f w))
          | some name, some f =>
              ...
              (res, st.mapWorld update)
          | some "&self", none => (res, st)
          | some _, none => (res, st)
...
def mettaEval (env : MinEnv) (fuel : Nat) (st : St) (bnd : Bindings) (a : Atom) : List (Atom × Bindings) × St :=
  ...
  | fuel + 1 =>
    ...
    | Atom.expr (Atom.sym op :: args) =>
      if env.profile.typecheckStrict && arityMismatch env op args then
        ([(Atom.expr [Atom.sym "Error", Atom.expr (Atom.sym op :: args),
            Atom.sym "IncorrectNumberOfArguments"], bnd)], st)
      else if let some (pos, expected, actual) :=
          (if env.profile.typecheckStrict then typeMismatch env st.world op args else none) then
    ...
    | w =>
        ...
        pairs.foldl (fun (a2 : List (Atom × Bindings) × St) p =>
          ...
          else if returnsAtom env w && (env.profile == heProfile || !isEmbeddedOp p.1) then
            (a2.1 ++ [p], a2.2)
