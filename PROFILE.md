...
Oracle discipline: the sole oracle for pettaProfile is the clean archive of
the pinned native PeTTa revision created by `scripts/run-petta-pinned.sh`.
Tracked or untracked edits in a developer's PeTTa checkout cannot alter the
differential result. The petta-he-profile DIV registry is a hypothesis catalog
measured on the `--he` lane — background reading only.
