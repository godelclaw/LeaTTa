...
def AlphaExtendsAbove
  ...
  ∃ suffix : List (LogicVar × String),
    larger = smaller ++ suffix ∧
      (∀ identity name, (identity, name) ∈ suffix →
...
theorem included
  ...
  rcases extension with ⟨suffix, largerEq, _generated, _referenceAbove,
    _executableAbove⟩
  intro pair member
  rw [largerEq]
  exact List.mem_append_left suffix member

/-- A freshly generated suffix cannot exist when the complete enlarged graph
is already strictly below the suffix's lower reference frontier.  This rules
out irrelevant source/anonymous identities in zero-width clause activations:
every newly appended key is generated by construction, so the two strict
bounds are contradictory. -/
theorem eq_of_generatedBelow
    {smaller larger : List (LogicVar × String)}
    {referenceFloor executableFloor : Nat}
    (extension :
      AlphaExtendsAbove smaller larger referenceFloor executableFloor)
    (below : GeneratedBelow referenceFloor (larger.map Prod.fst)) :
    larger = smaller := by
  rcases extension with
    ⟨suffix, largerEq, generated, referenceAbove, _executableAbove⟩
  have suffixNil : suffix = [] := by
    by_contra nonempty
    obtain ⟨⟨identity, name⟩, member⟩ :=
      List.exists_mem_of_ne_nil suffix nonempty
    obtain ⟨index, identityShape⟩ := generated identity name member
    subst identity
    have suffixFirst : .generated index ∈ suffix.map Prod.fst :=
      List.mem_map.mpr ⟨(.generated index, name), member, rfl⟩
    have largerFirst : .generated index ∈ larger.map Prod.fst := by
      rw [largerEq, List.map_append, List.mem_append]
      exact Or.inr suffixFirst
    exact (Nat.not_lt_of_ge (referenceAbove index suffixFirst))
      (below index largerFirst)
  rw [largerEq, suffixNil, List.append_nil]
...
theorem extendAbove
    ...
    AlphaAllocationGap larger referenceStart referenceEnd executableStart
      executableEnd := by
  rcases extension with
    ⟨suffix, largerEq, _generated, referenceAbove, executableAbove⟩
  rw [largerEq]
  apply gap.append
  constructor
  · intro index member
