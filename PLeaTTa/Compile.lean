...
/-- `C⟦e⟧ = (t, G)` with the fresh counter threaded. -/
partial def compileExpr (env : CEnv) (n : Nat) : Atom →
    CompileM (Atom × List Goal × Nat)
  | Atom.var v => .ok (Atom.var v, [], n)
  | Atom.sym s => .ok (canonBool (Atom.sym s), [], n)
  | Atom.gnd g => .ok (canonBool (Atom.gnd g), [], n)
  | Atom.expr [] => .ok (nilA, [], n)   -- () is the empty list
  ...
  | Atom.expr (Atom.sym h :: args) => compileApp env n h args
  | Atom.expr (Atom.var v :: args) => do
      -- first-class function value: dynamic application
      let (ts, gs, n1) ← compileList env n args
      let (r, n2) := fresh n1
      .ok (r, gs ++ [Goal.callDyn (Atom.var v) ts r], n2)
  | Atom.expr (hd :: args) => do
      ...
      let (th, gh, n1) ← compileExpr env n hd
      let (ts, gs, n2) ← compileList env n1 args
...
partial def compilePattern (env : CEnv) (n : Nat) : Atom →
    CompileM (Atom × List Goal × Nat)
  | Atom.var v => .ok (Atom.var v, [], n)
  | Atom.sym s => .ok (canonBool (Atom.sym s), [], n)
  | Atom.gnd g => .ok (canonBool (Atom.gnd g), [], n)
  ...
  | Atom.expr [Atom.sym "#c", h, t] =>
      .ok (Atom.expr [Atom.sym "#c", h, t], [], n)
  | Atom.expr [Atom.sym "cons", h, t] => do
      let (ph, gh, n1) ← compilePattern env n h
      let (pt, gt, n2) ← compilePattern env n1 t
      .ok (consC ph pt, gh ++ gt, n2)
  | Atom.expr (Atom.sym f :: args) =>
      if env.defined.contains f || env.isBin f || specialHead f then
        compileExpr env n (Atom.expr (Atom.sym f :: args))
      else do
        let (ts, gs, n1) ← compilePatternList env n args
        .ok (chainOf (Atom.sym f :: ts), gs, n1)
  | Atom.expr es => do
      let (ts, gs, n1) ← compilePatternList env n es
      .ok (chainOf ts, gs, n1)

partial def compileApp (env : CEnv) (n : Nat) (h : String) (args : List Atom) :
    CompileM (Atom × List Goal × Nat) := do
  if env.translatorRules.contains h then
    let ordinaryEnv :=
      { env with translatorRules := env.translatorRules.erase h }
    let (code, goals, n1) ← compileAppCore ordinaryEnv n h args
    let (r, n2) := fresh n1
    .ok (r, goals ++ [Goal.evalg code r], n2)
  else
    compileAppCore env n h args

partial def compileAppCore (env : CEnv) (n : Nat) (h : String)
    (args : List Atom) : CompileM (Atom × List Goal × Nat) := do
  match h, args with
  ...
  | "unquote", [Atom.expr [Atom.sym "quote", e]] => compileApp env n "eval" [e]
  ...
  | "test", [e, expected] => do
      ...
      let (te, ge, n1) ← compileExpr env n e
      let (answers, n2) := fresh n1
      let (tx, gx, n3) ← compileExpr env n2 expected
      let (r, n4) := fresh n3
  ...
  | "trace!", [message, value] =>
      ...
      compileApp env n "progn"
        [Atom.expr [Atom.sym "println!", message], value]
  | "and-then", [a, b] =>
      -- [SPEC translator.pl:177-180] evaluate `b` only when `a` is True.
      compileApp env n "if" [a, b, Atom.sym "False"]
  | "or-else", [a, b] =>
      -- [SPEC translator.pl:181-184] skip `b` when `a` is already True.
      compileApp env n "if" [a, Atom.sym "True", b]
  | "#+", [a, b] => compileApp env n "+" [a, b]   -- petta's flexible +
  | "#-", [a, b] => compileApp env n "-" [a, b]
  | "cons", [h, t] => do
      -- the list constructor IS structure: narrows via unification
      let (th, gh, n1) ← compileExpr env n h
      let (tt, gt, n2) ← compileExpr env n1 t
      .ok (consC th tt, gh ++ gt, n2)
  | "if", [c, t] => do
      let (tc, gc, n1) ← compileExpr env n c
      let (tt, gt, n2) ← compileExpr env n1 t
      .ok (tt, gc ++ [Goal.eq tc trueA] ++ gt, n2)
  | "if", [c, t, e] => do
      let (tc, gc, n1) ← compileExpr env n c
      let (tt, gt, n2) ← compileExpr env n1 t
      let (te, ge, n3) ← compileExpr env n2 e
      let (r, n4) := fresh n3
      .ok (r, gc ++ [Goal.ite tc (compileBranch r (tt, gt))
        (compileBranch r (te, ge)) r], n4)
  | "let", [p, v, b] => do
      -- [SPEC translator.pl:178-181] goal order is (Pv=V), Gp, Gv, Gi:
      -- the result terms unify FIRST (logic variables), then pattern goals
      -- (narrowing), then value goals, then the body
      let (tv, gv, n1) ← compileExpr env n v
      let (tp, gp, n2) ← compileExpr env n1 p
      let (tb, gb, n3) ← compileExpr env n2 b
  ...
  | "case", [scrut, Atom.expr arms] => do
      ...
      let (ts, gs, n1) ← compileExpr env n scrut
      let (sv, n2) := fresh n1
      let (r, n3) := fresh n2
      let emptyArm := arms.findSome? (fun arm =>
        ...
        | _ => none)
      let rec buildArms (m : Nat) : List Atom →
          CompileM (List Goal × Nat)
        | [] => .ok ([Goal.eq trueA falseA], m)   -- no arm matched: fail
        | Atom.expr [Atom.sym "Empty", _] :: more => buildArms m more
        | Atom.expr [p, body] :: more => do
            let (tp, gp, m1) ← compilePattern env m p
            let (tb, gb, m2) ← compileExpr env m1 body
            let (elseGs, m3) ← buildArms m2 more
            let carry := bindingTemplate tp [p]
            .ok ([Goal.softcut carry (gp ++ [Goal.eq tp sv])
                   (gb ++ [Goal.eq r tb]) elseGs], m3)
        | _ :: _ => .error "case: malformed arm"
      let (armGs, n4) ← buildArms n3 arms
      match emptyArm with
        | some body => do
            let (tb, gb, m) ← compileExpr env n4 body
  ...
  | "unify", [Atom.sym "&self", pat, thn, els] => do
      if !env.defined.contains "unify" then do
        ...
        let (tt, gt, n1) ← compileExpr env n thn
        let (te, ge, n2) ← compileExpr env n1 els
        let (r, n3) := fresh n2
        let p := chainify pat
  ...
  | "unify", [a, bb, thn, els] => do
      if !env.defined.contains "unify" then do
        let (ts, gs, n1) ← compileList env n [a, bb, thn, els]
        .ok (chainOf (Atom.sym "unify" :: ts), gs, n1)
      else
        -- soft cut: bindings from the successful unification reach `thn`
        let (ta, ga, n1) ← compileExpr env n a
        let (tb2, gb2, n2) ← compileExpr env n1 bb
        let (tt, gt, n3) ← compileExpr env n2 thn
        let (te, ge, n4) ← compileExpr env n3 els
        let (r, n5) := fresh n4
  ...
  | "succeedsPredicate", [Atom.expr (sp :: Atom.sym rel :: args)] => do
      ...
      let (ts, gs, n1) ← compileExpr env n sp
      let (r, n2) := fresh n1
  ...
  | "for", [v, collection, body] =>
      ...
      compileApp env n "let" [v, Atom.expr [Atom.sym "superpose", collection], body]
  | "chain", [e, v, body] => compileApp env n "let" [v, e, body]
  | "foldall", [f, gen, init] => do
      let (tg, gg, n1) ← compileExpr env n gen
      let (lst, n2) := fresh n1
      let (tf, gf, n3) ← compileExpr env n2 f
      let (ti, gi, n4) ← compileExpr env n3 init
      let (r, n5) := fresh n4
  ...
  | "transaction", [body] => do
      ...
      let (tb, gb, n1) ← compileExpr env n body
  ...
  | "union", [e1, e2] => do
      ...
      let (t1, g1, n1) ← compileExpr env n e1
      let (t2, g2, n2) ← compileExpr env n1 e2
      let (l1, n3) := fresh n2
      let (l2, n4) := fresh n3
      let (cc, n5) := fresh n4
      let (r, n6) := fresh n5
  ...
  | "call", [Atom.expr (Atom.sym f :: xs)] => do
      ...
      let (ts, gs, n1) ← compileArgs env n f xs
      let (r, n2) := fresh n1
      .ok (r, gs ++ [Goal.call f ts r], n2)
  | "reduce", [e] => compileApp env n "eval" [e]
  | "get-type-space", [_sp, e] => compileApp env n "get-type" [e]
  | "get-atoms", [sp] => do
      let (ts, gs, n1) ← compileExpr env n sp
      let (r, n2) := fresh n1
  ...
  | "=", [a, b] => do
      ...
      let (ta, ga, n1) ← compileExpr env n a
      let (tb, gb, n2) ← compileExpr env n1 b
      let (r, n3) := fresh n2
  ...
  | "add-atom", [sp, a] => do
      ...
      let (tsp, gsp, n0) ← compileExpr env n sp
      match a with
  ...
  | "bind!", [name, Atom.expr [Atom.sym "new-state", e]] => do
      ...
      compileApp env n "change-state!" [name, e]
  | "bind!", [_name, _value] =>
                ...
                let (ts, gs, m1) ← args.zip ptys |>.foldlM
                  (fun (a2 : List Atom × List Goal × Nat) (arg, ty) => do
                    if ty == Atom.sym "Expression" then
                      ...
                      let (t, g, m) ← compileExpr env a2.2.2 arg
...
/-- Compile application arguments, honoring the `Atom`-typed staging mask. -/
partial def compileArgs (env : CEnv) (n : Nat) (h : String) (args : List Atom) :
    CompileM (List Atom × List Goal × Nat) := do
  let mut ts : List Atom := []
  let mut gs : List Goal := []
  let mut m := n
  for (a, i) in args.zipIdx do
    if env.atomTyped h i then
      ...
      let (t, g, m') ← compileExpr env m a
      ts := ts ++ [t]; gs := gs ++ g; m := m'
  .ok (ts, gs, m)

partial def compileList (env : CEnv) (n : Nat) (es : List Atom) :
    CompileM (List Atom × List Goal × Nat) := do
  let mut ts : List Atom := []
  let mut gs : List Goal := []
  let mut m := n
  for e in es do
    let (t, g, m') ← compileExpr env m e
    ts := ts ++ [t]; gs := gs ++ g; m := m'
  .ok (ts, gs, m)

partial def compilePatternList (env : CEnv) (n : Nat) (es : List Atom) :
    CompileM (List Atom × List Goal × Nat) := do
  let mut ts : List Atom := []
  let mut gs : List Goal := []
  let mut m := n
  for e in es do
    let (t, g, m') ← compilePattern env m e
    ts := ts ++ [t]; gs := gs ++ g; m := m'
  .ok (ts, gs, m)
